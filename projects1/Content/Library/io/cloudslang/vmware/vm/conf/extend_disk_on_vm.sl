...
####################################################
#!!
#! @description:                 - Increases the size of a hard disk on a virtual machine.
...
#! @input device_number:         - Disk number as seen from VMWare client (Hard Disk 1, Hard Disk 2).
#!                                 Example: "Hard disk 1" would be device_number "1".
#! @input disk_size:             - The size of the disk for a virtual machine in GB. If it is less than the current disk
#!                                  size, the operation will complete succesfully, but the disk size will remain unmodified.
...
#! @output return_result:        - Task result or operation result. Reason for error on failure.
...
operation: 
   ...
   java_action:
      ...
      method_name: execute
      class_name: io.cloudslang.content.actions.vmware.vm.conf.ExtendDiskOnVM
   outputs: 
