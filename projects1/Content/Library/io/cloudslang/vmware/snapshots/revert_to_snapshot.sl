...
####################################################
#!!
#! @description:                 - Revert a virtual machine to an existing named snapshot or the most current one if none specified.
...
#!                                 Default: 800
#! @input snapshot_name:         - Name of snapshot. If the snapshot name is not provided the operation performs a revert
#!                                  to the current snapshot (one level up in the hierachy from the 'You are here' position).
...
operation: 
   ...
   java_action:
      ...
      method_name: execute
      class_name: io.cloudslang.content.actions.vmware.snapshots.RevertToSnapshot
   outputs: 
