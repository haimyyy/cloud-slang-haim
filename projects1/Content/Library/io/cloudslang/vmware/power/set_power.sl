...
####################################################
#!!
#! @description:                 - Sets the power state of virtual machine (poweredOn,poweredOff,suspended).
...
#! @input vm_power_state:        - Desired VM power state
#!                                 Valid values: "poweredOn", "poweredOff", "suspended"
#! @input wait_guest:            - If this is true the VM will wait for guest (requires VM tools), otherwise
#!                                  it will just power on/off the VM. If this input is provided retries and
#!                                  check_guest_running_delay must be provided.
#!                                 Default: true
#! @input retries:               - Number of checks if the VM Tools started. After these retries the flow will fail
#!                                 Default: 60
#! @input check_guest_running_delay:- The delays between checks if the VM Tools started. The value is in seconds.
#!                                  The total time before the flow fails is check_guest_running_delay * retries
#!                                   (default 300 seconds)
#!                                 Default: 5
...
#! @output return_result:        - The result of the flow, the failure message in case of error
#! @output return_result:        - The failure message in case of error
#! @output return_code:          - The return code of the operation. (0,-1)
#! @result SUCCESS:              - The machine power state has been set (return_code 0)
...
flow:
   ...
   -  keystore_password:
         ...
         required: false
   workflow:
   -  check_request_power_on:
         do:
            strings.string_equals:
            -  first_string: ${vm_power_state}
            -  second_string: "poweredOn"
            -  ignore_case: "true"
         navigate:
         -  SUCCESS: power_on_vm
         -  FAILURE: check_request_power_off
   -  power_on_vm:
         do:
            power.power_on:
            -  host
   ...
   -  power_off_vm:
         ...
         -  SUCCESS: SUCCESS
         -  FAILURE: FAILURE
   -  check_request_suspend:
         do:
            strings.string_equals:
            -  first_string: ${vm_power_state}
            -  second_string: "suspended"
            -  ignore_case: "true"
         navigate:
         -  SUCCESS: suspend_vm
         -  FAILURE: invalid_power_state_request
   -  suspend_vm:
         do:
            power.suspend:
            -  host
         ...
         -  SUCCESS: SUCCESS
         -  FAILURE: FAILURE
   -  invalid_power_state_request:
         do:
            math.add_numbers:
            -  value1: "1"
            -  value2: "1"
         publish:
            -  failure_message: 'Invalid power state request. Valid power states: poweredOn, poweredOff, suspended'
            -  return_code: "-1"
         navigate:
         -  SUCCESS: FAILURE
         -  FAILURE: FAILURE
   ...
   -  return_code
   -  failure_message
   -  return_result: ${'Successfully set the VM power state' if return_code=='0' else failure_message}
   results:
   - FAILURE
   - SUCCESS
